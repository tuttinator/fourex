...
"""
        ...
        try:
            ...
            if llm_response.content:
                try:
                    ...
                    if "actions" in plan_data and isinstance(
                        plan_data["actions"], list
                    ):
                    ...
                    plan = self._fallback_parse_plan(
                        llm_response.content, game_state, player_id
                    )
            else:
    ...
    def _fallback_parse_plan(
        self, content: str, game_state: GameState = None, player_id: str = ""
    ) -> TurnPlan:
            ...
            if json_match:
                ...
                if isinstance(parsed, dict):
                            ...
                            if action_type in action_type_mapping:
                                ...
                                try:
                                    ...
                                    if mapped_type == ActionType.MOVE:
                                        ...
                                        to_coord = action.get("to") or action.get(
                                            "destination"
                                        )
                                        ...
                                        if game_state and player_id:
                                            ...
                                            if my_units:
                                                ...
                                                if (
                                                    isinstance(to_coord, dict)
                                                    and "x" in to_coord
                                                    and "y" in to_coord
                                                ):
                                                    action_data["target_location"] = (
                                                        Coord(
                                                            x=int(to_coord["x"]),
                                                            y=int(to_coord["y"]),
                                                        )
                                    ...
                                    self.logger.warning(
                                        f"Failed to create action {mapped_type}: {e}"
                                    )
                                    continue
                    ...
                    return TurnPlan(
                        ...
                        priorities=(
                            priorities if isinstance(priorities, list) else ["Survive"]
                        ),
                    )
        except Exception as e:
...
class FourXAgent:
    ...
    def _convert_actions_to_api(
        ...
        non_pass_actions = [
            action for action in actions if action.type != ActionType.PASS
        ]
        ...
        for action in non_pass_actions:  # Only process non-pass actions
            try:
                if action.type == ActionType.MOVE:
                    ...
                    self.logger.warning(
                        f"Skipping unsupported action type: {action.type}"
                    )
                    continue
