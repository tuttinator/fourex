...
class PersistentGameClient(GameClient):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if e.response.status_code == 404:
                    self.logger.error(
                        "Game not found (404) - not retrying", error=str(e)
                    )
                    return None

                self.logger.error(
                    "HTTP error getting game state", error=str(e), attempt=attempt + 1
                )
                if attempt < max_retries - 1:
    ...
    def check_game_persistence(self, game_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if e.response.status_code == 404:
                self.logger.debug(
                    "Game not found (404) while checking persistence", game_id=game_id
                )
            else:
