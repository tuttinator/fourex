...
class MilitaryAnalysisRequest(BaseModel):
    ...
    include_predictions: bool = Field(
        default=True,
        description="Include combat outcome predictions"
    )
...
async def get_game_state(request: GameStateRequest) -> str:
    ...
    try:
        ...
        visible_units = {
            uid: unit for uid, unit in game_state.units.items()
            if (unit.loc.x, unit.loc.y) in visible_tiles
        }
        visible_cities = {
            cid: city for cid, city in game_state.cities.items()
        ...
        state_summary = {
            ...
            "map_size": {"width": game_state.map_width, "height": game_state.map_height},
            ...
            "my_resources": game_state.stockpiles.get(request.player_id, {
                "food": 0, "wood": 0, "ore": 0, "crystal": 0
            }),
    ...
    try:
        ...
        for tile in neutral_tiles:
            if tile.terrain in ["plains", "forest"]:  # Good for cities
                nearby_resources = sum(1 for rt in resource_tiles 
        ...
        analysis = {
            ...
            "expansion_opportunities": sorted(expansion_opportunities, 
                                            key=lambda x: x["nearby_resources"], reverse=True)[:5],
...
async def evaluate_military_position(request: MilitaryAnalysisRequest) -> str:
    ...
    logger.info("Evaluating military position", game_id=request.game_id, player_id=request.player_id)
    ...
    try:
        ...
        my_units = [u for u in game_state.units.values() 
                   if u.owner == request.player_id and (u.loc.x, u.loc.y) in visible_tiles]
        enemy_units = [u for u in game_state.units.values() 
                      if u.owner != request.player_id and (u.loc.x, u.loc.y) in visible_tiles]
                    ...
                    if city.owner == request.player_id:
                        ...
                        if distance <= 3:  # Within threat range
                            threats.append({
                                ...
                                "enemy_location": {"x": enemy_unit.loc.x, "y": enemy_unit.loc.y},
        ...
        for enemy_unit in enemy_units:
            nearby_my_units = [u for u in my_units 
                             if abs(u.loc.x - enemy_unit.loc.x) + abs(u.loc.y - enemy_unit.loc.y) <= 2]
            if len(nearby_my_units) > 1:  # We can potentially overwhelm
                opportunities.append({
                    "type": "attack_opportunity",
                    "target": enemy_unit.type,
                    "target_location": {"x": enemy_unit.loc.x, "y": enemy_unit.loc.y},
        ...
        analysis = {
            ...
            "unit_breakdown": {
                "my_units": {unit.type: len([u for u in my_units if u.type == unit.type]) 
                           for unit in my_units},
                "enemy_units": {unit.type: len([u for u in enemy_units if u.type == unit.type]) 
...
async def find_resource_opportunities(request: ResourceOpportunitiesRequest) -> str:
    ...
    logger.info("Finding resource opportunities", game_id=request.game_id, player_id=request.player_id)
...
async def validate_actions(request: ActionValidationRequest) -> str:
    ...
    logger.info("Validating actions", game_id=request.game_id, player_id=request.player_id, 
...
def _generate_military_assessment(my_strength: int, enemy_strength: int, threats: list, opportunities: list) -> str:
    ...
    opportunity_text = f" {len(opportunities)} attack opportunities available." if opportunities else " No clear attack opportunities."
...
def _generate_resource_advice(opportunities: list, game_state, player_id: str) -> str:
    ...
    if food < 20:
        ...
        if food_ops:
            advice.append(f"Low food ({food}). Priority: develop food site at ({food_ops[0]['location']['x']},{food_ops[0]['location']['y']}).")

    if wood < 10:
        wood_ops = [o for o in opportunities if o["resource"] == "wood"]
        if wood_ops:
            advice.append(f"Low wood ({wood}). Consider wood site at ({wood_ops[0]['location']['x']},{wood_ops[0]['location']['y']}).")

    crystal_ops = [o for o in opportunities if o["resource"] == "crystal"]
    if crystal_ops:
        advice.append(f"Crystal available at ({crystal_ops[0]['location']['x']},{crystal_ops[0]['location']['y']}) - high priority.")
...
def _validate_single_action(game_state, action: dict[str, Any], player_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if action_type == "MOVE":
            ...
            if unit.owner != player_id:
                return {"valid": False, "reason": "Cannot move unit not owned by player"}
            ...
            if city.owner != player_id:
                return {"valid": False, "reason": "Cannot train units in city not owned by player"}
