...
def _make_test_state() -> tuple[GameState, GameState]:
    ...
    tid = 0
    terrains = [
        [Terrain.PLAINS, Terrain.PLAINS, Terrain.FOREST, Terrain.MOUNTAIN, Terrain.WATER],
        [Terrain.PLAINS, Terrain.PLAINS, Terrain.PLAINS, Terrain.FOREST, Terrain.WATER],
        [Terrain.FOREST, Terrain.PLAINS, Terrain.PLAINS, Terrain.PLAINS, Terrain.MOUNTAIN],
        [Terrain.WATER, Terrain.FOREST, Terrain.PLAINS, Terrain.PLAINS, Terrain.PLAINS],
        [Terrain.WATER, Terrain.WATER, Terrain.MOUNTAIN, Terrain.PLAINS, Terrain.PLAINS],
    ]
    for y in range(5):
        for x in range(5):
            tiles.append(
                Tile(id=tid, loc=Coord(x=x, y=y), terrain=terrains[y][x])
            )
            tid += 1
    ...
    units = {
        1: Unit(id=1, owner="alice", type=UnitType.SCOUT, hp=2, moves_left=3, loc=Coord(x=1, y=1)),
        2: Unit(id=2, owner="bob", type=UnitType.SOLDIER, hp=4, moves_left=2, loc=Coord(x=3, y=3)),
    }
...
class TestRenderAsciiPure:
    ...
    def test_fog_of_war_applied(self):
        ...
        map_lines = [l for l in lines if "|" in l and l.strip()[0].isdigit()]
