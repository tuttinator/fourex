...
export default function ReplayPage() {
  ...
  //
  // ``placeholderData: keepPreviousData`` keeps the last turn's snapshot
  // mounted while the next one is fetched. Without it, the
  // ``stateLoading`` branch unmounted ``PixiMap`` between turns and its
  // viewport (zoom + pan) reset every scrub — frustrating when the
  // whole point of stepping through turns is watching units move.
  const {
    data: turnState,
    isFetching: stateFetching,
    isPlaceholderData: stateIsStale,
    error: stateError,
  } = useQuery({
        ...
        <div className="flex-1 relative">
          {turnState ? (
            <>
              ...
              />
              {stateFetching && stateIsStale && (
                <div className="absolute top-3 right-3 flex items-center gap-2 rounded-md border bg-background/80 backdrop-blur px-2.5 py-1.5 text-xs text-muted-foreground shadow-sm">
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                  Loading turn {effectiveTurn}…
                </div>
              )}
            </>
          ) : stateError ? (
