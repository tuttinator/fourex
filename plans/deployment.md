...
- [ ] Branch protection on `main` requires passing CI before merge **— partially configured via a Ruleset (not classic branch protection; `gh api repos/tuttinator/fourex/branches/main/protection` returns `404 Branch not protected` because that endpoint doesn't surface Rulesets). Ruleset `15504879` "Default branches" (`enforcement: active`, targets `~DEFAULT_BRANCH` + `refs/heads/main`) already enforces `deletion`, `non_fast_forward`, and `pull_request` (PR required, 0 approvals, all merge methods). The remaining gap is a `required_status_checks` rule naming `CI / CI` (the meta-job, `name: CI`). Do **not** require `Backend` / `Frontend` directly — they legitimately `skipped` on path-filter misses and a required check in `skipped` state blocks merging. Successful `CI` runs have been observed on `main` (e.g. runs 24872148015, 24881928615, 24881943296, 24893966563, 24894210316) so the check is selectable in the Settings UI.**
