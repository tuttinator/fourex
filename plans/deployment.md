...
- [x] Branch protection on `main` requires passing CI before merge — configured via repo Ruleset `15504879` "Default branches" (`enforcement: active`, targets `~DEFAULT_BRANCH` + `refs/heads/main`, not classic branch protection — `gh api repos/tuttinator/fourex/branches/main/protection` will continue to report `404 Branch not protected` because that endpoint does not surface Rulesets; use `gh api repos/tuttinator/fourex/rulesets/15504879` instead). Rules enforced: `deletion`, `non_fast_forward`, `pull_request` (PR required, 0 approvals, all merge methods), and `required_status_checks` pinned to context `CI` (integration 15368 = GitHub Actions — the `ci.yml` meta-job, deliberately not the path-filtered `Backend` / `Frontend` jobs which legitimately `skip` and would otherwise block merging).
