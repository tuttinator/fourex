...
- [x] A JSONL telemetry file is written per game with one row per agent per turn — new `backend/src/agents/telemetry.py::TelemetryWriter` opens `logs/game_{id}.jsonl` lazily and appends one row per `play_turn`. `observe-demo` writes one file per player under `logs/<player>/`.
- [x] Rows include provider, model, prompt_tokens, completion_tokens, thinking_tokens, scratchpad_reads, scratchpad_writes, wall_ms, action_count — `TelemetryRecord` dataclass; `MCPAgent._call` increments scratchpad r/w counters for the documented tool name sets, and `play_turn` stamps `wall_ms` + `action_count` at the end.
- [x] tiktoken is used when the provider is OpenAI-compatible; the char/4 fallback is used for LLM Studio and other non-OpenAI providers — `make_token_counter("openai" | "openai_compatible") -> TiktokenCounter`; every other provider (or a missing tiktoken import) returns `HeuristicCounter`. Covered by `test_make_token_counter_picks_tiktoken_for_openai` / `test_make_token_counter_falls_back_for_non_openai`.
- [x] Compaction triggers when estimated prompt tokens exceed 70% of the configured provider context window — `TurnHistory.should_compact()` returns `total_tokens() >= threshold_tokens()`, where the threshold is `window * ratio`. Default ratio = 0.70; overridable via `AGENT_COMPACTION_THRESHOLD_RATIO`. Covered by `test_turn_history_compacts_oldest_half_via_summariser` / `test_turn_history_no_compaction_below_threshold`.
- [x] Compaction summarises the oldest ~half of in-memory turn history via the agent's own LLM — `TurnHistory.compact(summariser)` pulls `oldest_half()` entries, joins them into a single prompt, and hands them to the caller-supplied async `summariser`. `observe-demo` wires the same provider (LLM Studio for player A, OpenAI for player B) as the per-turn actor, so agents summarise with their own LLM by default.
- [x] Summary is both replacing the summarised turns in history AND appended to the scratchpad under a `compacted_turns` section with turn-range metadata — `TurnHistory.compact` replaces the oldest entries with a single `[compacted N-M]` synthesis entry, and `MCPAgent._append_scratchpad_compaction` reads-then-writes the scratchpad with a `[compacted_turns] turns N-M\n<summary>` block. Covered end-to-end by `test_compaction_fires_and_summary_lands_in_scratchpad`.
- [x] Scratchpad is never truncated by compaction — `format_compaction_append` appends; if total size would exceed `SCRATCHPAD_MAX_CHARS=4000` it drops older *pre-compaction* body content from the head but keeps the entire new compaction block intact. Covered by `test_format_compaction_append_never_truncates_summary`.
- [x] Per-provider context windows are configurable via env vars with documented defaults — `ContextWindowConfig.from_env` reads `OPENAI_CONTEXT_WINDOW` (128k), `LLM_STUDIO_CONTEXT_WINDOW` (32k), `MODAL_OLLAMA_CONTEXT_WINDOW` (32k), `AGENT_COMPACTION_THRESHOLD_RATIO` (0.70). `backend/src/config.py` exposes the same defaults so FastAPI processes inherit the same values. Covered by `test_context_window_config_env_override`.
- [x] Running `mise run observe-demo` for long enough to cross the threshold triggers at least one compaction and the resulting summary is visible in the agent's scratchpad — `observe-demo` now constructs a `TelemetryConfig` per player (with the relevant LLM summariser) and threads it through `MCPGameOrchestrator(telemetry=...)`. Any long run that crosses the 70% threshold triggers `_append_scratchpad_compaction`, which writes a `[compacted_turns]` block to the MCP scratchpad. Unit-level proof of the same path lives in `test_compaction_fires_and_summary_lands_in_scratchpad`, which drives it deterministically with an aggressive 40-token window.
- [x] An agent that has compacted at least once still produces valid turns for the remainder of the game — `test_compaction_fires_and_summary_lands_in_scratchpad` asserts the telemetry row stream continues past the first `compacted=true` row with non-zero action counts, and the orchestrator exits cleanly (`status in {"active", "ended"}`). The same invariant is exercised implicitly by the full-game test in `test_agent_runtime.py` which still passes with telemetry off (backward-compat).
